// Quant Desk — backtest engine (slice 1). Verbatim lineage: loops/backtest/engine.js
// (Gold Metal Alchemist local backtest engine v1) plus four backward-compatible additions:
//
//   1. params.entryGate  — optional (ctx) => boolean, consulted immediately before a position
//                          is opened. `false` skips the entry WITHOUT consuming the arm's
//                          entriesLeft. Null/undefined gate = original behaviour.
//   2. params.countFrom  — optional unix seconds. Every bar is still simulated (warm-up), but
//                          the returned `trades` only contain trades whose entryT >= countFrom.
//                          The result also carries `warmupBars` (M1 bars before countFrom).
//   3. Trade records gain risk, entryIdx/exitIdx (LTF bar indexes), pt_date, week, month
//                          (all in PT, PT = server − 10h; keyed on the EXIT time = when the R
//                          is realized; entry_pt_date is included for reference).
//   4. Exports DEFAULTS (fully resolved default params) and PARAM_SCHEMA (desk/engine/schema.json).
//
// Nothing here executes trades or calls a model. Demo/research only.
// Every structural definition here is a PARAMETER — Mike calibrates them as we go.

const fs = require('fs');
const path = require('path');

const PARAM_SCHEMA = require(path.join(__dirname, 'schema.json'));
...
function ptHour(t) {
  const d = new Date((t - PT_OFFSET_S) * 1000);
...
// ── PT calendar keys (same PT = server − 10h convention as ptHour) ────────────
function ptDateObj(t) { return new Date((t - PT_OFFSET_S) * 1000); }
const pad2 = (n) => String(n).padStart(2, '0');
// 'YYYY-MM-DD' calendar date in PT
function ptDate(t) {
  const d = ptDateObj(t);
  return `${d.getUTCFullYear()}-${pad2(d.getUTCMonth() + 1)}-${pad2(d.getUTCDate())}`;
}
// ISO-8601 week key 'YYYY-Www' (weeks start Monday; week 1 contains Jan 4) of the PT date
function weekKey(t) {
  const d = ptDateObj(t);
  const date = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()));
  const dayNum = date.getUTCDay() || 7; // Mon=1 … Sun=7
  date.setUTCDate(date.getUTCDate() + 4 - dayNum); // shift to the Thursday of this ISO week
  const yearStart = Date.UTC(date.getUTCFullYear(), 0, 1);
  const week = Math.ceil(((date.getTime() - yearStart) / 86400000 + 1) / 7);
  return `${date.getUTCFullYear()}-W${pad2(week)}`;
}
// 'YYYY-MM' of the PT date
function monthKey(t) {
  const d = ptDateObj(t);
...
// ── Defaults ───────────────────────────────────────────────────────────────────
// BASE = the literal default block of the original engine (applied verbatim: P = {...BASE, ...params}).
const BASE = {
  htf: 15, ltf: 3,
...
// IMPLICIT = the values the original engine falls back to (via ??, ||, !== false) when a
// param is absent. Together with BASE they form DEFAULTS: the fully-resolved default params.
// NOTE: the original derives armMode from requireSweep when armMode is absent
// ('sweep' if requireSweep else 'either'); DEFAULTS records the resolved value 'either'.
const IMPLICIT = {
  armMode: 'either',
  maxConcurrent: 1,
  addGateR: null,
  beAtR: null,
  liquidityLookbackDays: 5,
  tpBufferUsd: 0.5,
  minProfitRForOppExit: 0,
  useOppBos: false,
  useTerminalTp: true,
  trailActivateR: 0,
  useEmaExit: false,
  ...
  emaProxUsd: 0.5,
  emaMinProfitR: 0,
  emaOnlyWhenNoTerminal: false,
  exitBosStrength: null,     // null → same as pivotStrengthLtf
  trailMode: 'pivot',
  trailPivotStrength: null,  // null → same as pivotStrengthLtf
  atrMult: 3,
};
const DEFAULTS = Object.freeze({ ...BASE, ...IMPLICIT, windows: BASE.windows.map((w) => ({ ...w })) });
...
//
// Returns { trades, params, warmupBars }.
//   params.entryGate (fn|null)  — see header note 1.
//   params.countFrom (unix s)   — see header note 2.
function runDoubleBOS(m1, params) {
  const P = { ...BASE, ...params };
  const entryGate = typeof P.entryGate === 'function' ? P.entryGate : null;
  const countFrom = P.countFrom != null && Number.isFinite(+P.countFrom) ? +P.countFrom : null;
  ...
  for (let i = 0; i < ltfBars.length; i++) {
    ...
    for (let pi = positions.length - 1; pi >= 0; pi--) {
      ...
      if (emaExit != null && !hitSL) {
        trades.push(close(open, emaExit, bar.t, 'ema50_15m', P, i));
        positions.splice(pi, 1);
      } else if (hitSL) {
        trades.push(close(open, open.sl, bar.t, open.sl === open.initialSl ? 'sl' : 'trail_stop', P, i));
        positions.splice(pi, 1);
      } else if (oppBosExit) {
        trades.push(close(open, bar.c, bar.t, 'opp_bos', P, i));
        positions.splice(pi, 1);
      } else if (hitTP) {
        trades.push(close(open, open.tp, bar.t, 'tp', P, i));
        positions.splice(pi, 1);
      } else if (windowOver) {
        trades.push(close(open, bar.c, bar.t, 'window_close', P, i));
        positions.splice(pi, 1);
      }
    }
    ...
    // ── Entry gate (desk addition) ──────────────────────────────────────────────
    // Consulted immediately before the position opens. A `false` skips this entry only:
    // the arm keeps its entriesLeft and stays active for a later trigger.
    // ctx.tradesSoFar is the live array of CLOSED trade records (close order) — read-only.
    // ctx.armFromT identifies the arm (extra field beyond the contract; useful for tests).
    if (entryGate) {
      const ctx = {
        t: bar.t, dir: ev.dir, session: win.name, armType: arm.armType,
        entry, sl, risk,
        tradesSoFar: trades, openCount: positions.length,
        armFromT: arm.fromT,
      };
      if (!entryGate(ctx)) continue;
    }

    positions.push({ dir: ev.dir, armType: arm.armType, entry, sl, initialSl: sl, tp, pools, tapped: false, risk, entryT: bar.t, entryIdx: i, window: win, session: win.name, hh: bar.h, ll: bar.l });
  ...
  const lastIdx = ltfBars.length - 1;
  for (const open of positions) trades.push(close(open, ltfBars[lastIdx].c, ltfBars[lastIdx].t, 'data_end', P, lastIdx));

  // ── countFrom (desk addition): warm-up bars are simulated but not counted ─────
  let counted = trades;
  let warmupBars = 0;
  if (countFrom != null) {
    counted = trades.filter((tr) => tr.entryT >= countFrom);
    for (let k = 0; k < m1.length; k++) { if (m1[k].t >= countFrom) break; warmupBars++; }
  }

  return { trades: counted, params: P, warmupBars };
}

function close(pos, price, t, reason, P, exitIdx) {
  ...
  return {
    ...
    reason,
    // desk additions
    risk: +pos.risk.toFixed(2),          // price distance entry→initial SL
    initial_sl: +pos.initialSl.toFixed(2), // the stop at entry (sl above is the final, trailed stop)
    entryIdx: pos.entryIdx,              // LTF bar index of entry
    exitIdx: exitIdx == null ? null : exitIdx, // LTF bar index of exit
    pt_date: ptDate(t),                  // PT calendar date the R was REALIZED (exit)
    entry_pt_date: ptDate(pos.entryT),   // PT calendar date of entry (reference)
    week: weekKey(t),                    // ISO week of the exit, in PT ('YYYY-Www')
    month: monthKey(t),                  // 'YYYY-MM' of the exit, in PT
  };
}
...
module.exports = {
  // unchanged names
  loadM1, aggregate, findPivots, bosEvents, runDoubleBOS, metrics, ptHour,
  // desk additions
  DEFAULTS, PARAM_SCHEMA,
  inWindow, ptDate, weekKey, monthKey,
};
